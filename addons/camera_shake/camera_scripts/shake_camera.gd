...
func _processshake(angle_center : Vector3, delta : float) -> Vector3:
	...
	newRotate.x = maxYaw * shake * _get_noise(G.rng.randi(), delta)
	newRotate.y = maxPitch * shake * _get_noise(G.rng.randi(), delta + 1.0)
	newRotate.z = maxRoll * shake * _get_noise(G.rng.randi(), delta + 2.0)
